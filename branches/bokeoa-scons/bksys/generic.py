...
class genobj:
	...
	def __init__(self, val, env):
		...
		self.source=''
		## list of external source files (used in really special case) 
		self.extsource=''
		## target name 
		self.target=''
	...
	def execute(self):
		...
		if self.env['WINDOWS'] and self.type in ['shlib','kioslave','module']:
		...
		if self.type=='shlib' or self.type=='kioslave' or self.type=='module':
			...
			objects = self.env.Object(self.p_localsource)
			for extsrc in self.extsource:
				objects.append(self.env.Object(extsrc[10:-4],extsrc))
			ret=self.env.Program(self.p_localtarget, objects)
			...
			ret=self.env.bksys_staticlib(self.p_localtarget, self.p_localsource,self.instdir,self.libprefix)
