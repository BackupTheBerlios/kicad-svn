...
int net_number;
	...
	// Sortie de la dimension hors tout du pcb (dimensions + marge + pas_route)
#define B_MARGE 1000       // en 1/10000 inch
	fprintf(outfile,"j %d %d %d %d",
		(-B_MARGE - pas_route + m_Pcb->m_BoundaryBox.GetX() )/PSCALE,
		(-B_MARGE - pas_route + m_Pcb->m_BoundaryBox.GetY() )/PSCALE,
		(B_MARGE + pas_route + m_Pcb->m_BoundaryBox.GetRight() )/PSCALE,
		(B_MARGE + pas_route + m_Pcb->m_BoundaryBox.GetBottom() )/PSCALE);
	...
	fprintf(outfile," %d", pas_route / PSCALE);
...
/**************************************************************************/
/* void ReturnNbViasAndTracks(int netcode, int * nb_vias, int *nb_tracks) */
/**************************************************************************/
...
TRACK * track;
	...
	*nb_tracks = 0;
	...
	/************************************************/
	/* static void GenExistantTracks(FILE *outfile) */
	/************************************************/
...
FILE * File;
	...
	if( (File = wxFopen(FullFileName, wxT("rt"))) == NULL )
		...
		{
		 msg = _("Lecture fichier de routage ") + FullFileName;
		 Affiche_Message(msg);
		}
