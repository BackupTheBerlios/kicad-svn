...
int distmin;
	...
	{
		fdisty = (float)(cy - (row * pas_route));
		fdisty *= fdisty;
		for(col = col_min; col <= col_max; col++)
		{
			fdistx = (float)(cx - (col * pas_route) );
			fdistx *= fdistx;
...
void (*WriteCell)(int,int,int,BoardCell);
	...
	if( x0 == x1 )  // ligne verticale
	{
		...
		dy = y0 / pas_route; lim = y1 / pas_route ;
		dx = x0 / pas_route;
	...
	if( y0 == y1 )  // ligne horizontale
	{
		...
		dx = x0 / pas_route; lim = x1 / pas_route ;
		dy = y0 / pas_route;
		/* Clipping aux limites du board */
	...
	if ( abs(x1-x0) >= abs(y1-y0) ) /* segment peu incline */
	{
		...
		il = lim - dx; cumul = il / 2; delta = abs(y1 - y0)/pas_route;
		for( ; dx <= lim ; )
		{
			if( (dx >= 0) && (dy >= 0) &&
				(dx < Ncols) && (dy < Nrows) )
	...
	else
	{
		...
		il = lim - dy; cumul = il /2; delta = abs(x1 - x0)/pas_route;
		for( ; dy <= lim ; )
		{
			if( (dx >= 0) && (dy >= 0) &&
				(dx < Ncols) && (dy < Nrows) )
...
void (*WriteCell)(int,int,int,BoardCell);
	...
	/* Calcul des coord limites des cellules appartenant au rectangle */
	row_max = (cy + rayon) / pas_route;
	col_max = (cx + rayon) / pas_route;
	row_min = (cy - rayon) / pas_route; if (uy0 > row_min*pas_route ) row_min++;
	col_min = (cx - rayon) / pas_route; if (ux0 > col_min*pas_route ) col_min++;
	...
	for( row = row_min ;row <= row_max ; row++)
		...
		for(col = col_min;col <= col_max; col++)
			{
			rotrow = row * pas_route; rotcol = col * pas_route;
...
int cx, cy, dx, dy;
	...
	col_min = (ux0 - lg) / pas_route;
	if (col_min < 0 ) col_min = 0;
	col_max = (ux1 + lg + demi_pas) / pas_route;
	if (col_max > (Ncols-1) ) col_max = Ncols-1;

	if(inc > 0)
	{
		row_min = (uy0 - lg) / pas_route;
		row_max = (uy1 + lg + demi_pas) / pas_route;
	}
	else{
		row_min = (uy1 - lg) / pas_route;
		row_max = (uy0 + lg + demi_pas) / pas_route;
		}
	...
	{
		int cxr;
		cxr = (col * pas_route) - ux0;
		for(row = row_min; row <= row_max ;  row ++ )
		{
			cy = (row * pas_route) - uy0;
			cx = cxr;
...
//	else	// Horizontal segment
	...
	row_min = uy0 / pas_route;
	if (uy0 > row_min*pas_route ) row_min++; // Traitement de l'arrondi par defaut

	col_min = ux0 / pas_route;
	if (ux0 > col_min*pas_route ) col_min++;
